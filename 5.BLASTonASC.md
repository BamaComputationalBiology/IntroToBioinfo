...
We are going to make our own set of *C. elegans* miRNAs to search against. We do this by extracting all *C. elegans* miRNAs. We will use a new UNIX tool, grep, which stands for *globally search a regular expression and print*. UNIX grep is useful for searching and extracting specific text patterns which we call “regular expressions.” UNIX grep has tons of options and uses; we will just scratch the surface. Because UNIX grep can be complex I will give you the commands for extracting the sequences and redirecting to a new file. Identify what the options are doing through the grep manual. 

$ grep -A 1 “Caenorhabditis elegans” mature.fa | grep -v “^— -“ > Celegans_miRNA.fasta

The second part above should be an up arrow followed by two dashes if it’s not coming through clearly on the pdf.
...
Command-line blast has a parameter, ‘word_size’ that is not optimized for short sequences. Edit your script so the blastn line looks like this 
...
$ grep -A 1 ‘mir-35[a-z]’ Celegans_miRNA.fasta
...
6) How many BLAST hits do you get between *C.latens*-*C.elegans* and what is the average level of sequence identity (percent match)? If you were looking at just your *C.latens*-*C.elegans* BLAST results, what conclusions would you make about how miRNAs evolve? For example, is there a high level of sequence identity (slow evolution) or low (fast evolution)?
