...
Our data are currently integers that are raw counts of transcripts for each gene. Raw transcript counts are difficult to work with because it is difficult to understand 4 transcripts vs. 13 transcripts without some sense of scale. In order to introduce a scale we need to transform our data to log-fold changes. Then we can understand a doubling in expression (2-fold change) or a tripling in expression (3-fold change). Note that when you do this to visualize your data it is for diagnostics only; DESeq2 does this transformation in its calculations. 
