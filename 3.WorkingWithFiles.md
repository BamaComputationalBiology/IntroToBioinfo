...
First, log on to the ASC (reference tutorial 1 for directions). You should see a new occupant in your home directory, a directory called **shared**. We will use this directory for sharing data files. 
...
and remember to use ‘q’ to quit looking at the manual.
...
You should see the name of the file change
...
This is a *fastq* file. It has four lines for each DNA sequencing read which are 1) read identifier; 2) DNA sequence (or read); 3) "+" and 4) Quality for each base. We will go into more depth about fastq files later but for now we will use this file to learn more shell commands. 
...
You have now created a small file for testing. 
...
What happens (Hint: Check how many lines are in new.fastq with $ wc -l new.fastq )?
