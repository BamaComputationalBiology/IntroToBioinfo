...
We are analyzing a file of DNA sequence from a recent Illumina HiSeq run (this is only part of the file). Sequencing files are very large so we compress them to save space. The file should be in the **class_shared** directory in your home directory. You can read things in **class_shared** but you can’t edit so copy it from **class_shared** into your home directory. There are two ways to navigate into the **class_shared** directory. First, you can get to it from your directory:
